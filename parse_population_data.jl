...
include(joinpath(project_dir, "common.jl"))
...
function compute_state_county_data(
        ...
        if geometry == "Polygon"
            for polygon in block_json["geometry"]["coordinates"]
                for coord in polygon
                    long::Float64 = deg_to_rad * Float64(coord[1])
                    lat::Float64 = deg_to_rad * Float64(coord[2])
    ...
    for (i, (geoid, county_sums)) in enumerate(sums)
        ...
        state_county_data[i, 1] = Int64(geoid)
        state_county_data[i, 2] = Int64(county_votes[1])
        state_county_data[i, 3] = Int64(county_votes[2])
        state_county_data[i, 4] = Int64(county_votes[3])
